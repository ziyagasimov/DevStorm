...
const SpeakerDashboard = () => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("speaker_profiles" as any).select("*").eq("user_id", user.id).single()
      .then(({ data }) => {
        if (data) {
          const p = data as any as SpeakerProfile;
          setProfile(p);
          setForm(p);
        }
        setLoading(false);
  ...
  const handleSave = async () => {
    if (!user) return;
    setSaving(true);
    const { error } = await supabase.from("speaker_profiles" as any)
      .update({
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input type="number" value={form.years_of_experience} onChange={e => setForm({ ...form, years_of_experience: Number(e.target.value) })} />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label>Təcrübə</Label>
